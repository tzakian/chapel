...
proc plotToFile(NumSteps: [?Dom], outfile) {
  ...
  select (imgType) {
      when imageType.bw {
          for i in Dom.dim(1) {
              /*var start = outfile.offset();*/
              for j in Dom.dim(2) {
                  outfile.write(if NumSteps[i,j] then 0 else 1, " ");
              }
              outfile.writeln();
              /*writeln("real_start = ", start, " real_end = ", outfile.offset(), " start = ", i*cols*2+offset+i, " end = ", (i + 1)*cols*2+offset+i+1);*/
          }
      }
      ...
      when imageType.color {
          for i in Dom.dim(1) {
              for j in Dom.dim(2) {
                  outfile.writef("## ## ## ", (maxColor*NumSteps[i,j])/maxSteps,  0,  0);
              }
              outfile.writeln();
          }
      }
